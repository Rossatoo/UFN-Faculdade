...
select *, format(salario + (salario * 15) / 100, 2) as "Aumento de 15%"
from FUNCIONARIO, DEPARTAMENTO
where Dnome = "Administração" AND Dnumero = Dnr
order by salario;
...
select Projnumero, Projnumero, Dnumero, Unome, endereco, DataNasc
from funcionario
inner join departamento
on cpf = Cpf_gerente
inner join projeto
